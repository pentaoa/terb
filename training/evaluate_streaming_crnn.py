"""Evaluate a CRNN checkpoint using true state-carrying streaming inference."""
...
from train_streaming_crnn import load_model
...
def main():
    p = argparse.ArgumentParser(); p.add_argument("manifest"); p.add_argument("checkpoint")
    p.add_argument("--split", default="validation"); p.add_argument("--chunk", type=int, default=5); p.add_argument("--out", required=True)
    a = p.parse_args(); device = "cuda" if torch.cuda.is_available() else "cpu"
    model = load_model(a.checkpoint, device).eval()
    rows = [json.loads(x) for x in open(a.manifest) if x.strip() and json.loads(x)["split"] == a.split]
    result = []
    for row in rows:
