...
Music-reactive themes include Aurora, Sonic Texture, Miku, Square Album, and Circle Album. Aurora follows frequency position, spectrum energy, centroid, and transient flux. Sonic Texture samples a pitch-stable two-dimensional color field inside each Braille cell. Miku loops the bundled Tenor GIF in the center of the spectrum, contain-scaled so it never crops or overflows; the background is darkened and spectrum hits brighten the sampled GIF colors. Album themes poll system now-playing artwork first, fall back to Music or Spotify artwork, then use the active now-playing or CoreAudio-output app icon when no cover is available; music-player app icons are skipped so real covers stay authoritative. The cover/icon area is always rendered in Braille, with spectrum hits brightening the sampled colors. Square Album contain-scales the cover across the full spectrum canvas. Circle Album crops the cover into a centered disc and rotates the whole cover slowly as one surface. Miku playback starts at 5fps and adds 0.2x speed for every accent trigger in the last 3 seconds, with no cap. Their color drivers are time-smoothed so pitch and transient changes move without harsh jumps.
